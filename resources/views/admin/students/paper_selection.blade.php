    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Assigning academic parameters for
                <strong>{{ $student->full_name }}</strong> ({{ $student->reg_no }})</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="programme_id" required
                            ...
                            @foreach ($programmes as $prog)
                                <option
                                    value="{{ $prog->programme_id }}" {{ ($paperSelection && $paperSelection->programme_id == $prog->programme_id) ? 'selected' : '' }}>
                                    {{ $prog->code }} - {{ $prog->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        class="border border-yellow-400 w-full p-2.5 rounded outline-none focus:ring-2 focus:ring-purple-500 bg-white shadow-sm disabled:bg-gray-100 disabled:border-gray-300 disabled:text-gray-500">
                            <option value="" disabled {{ !$paperSelection ? 'selected' : '' }}>Select Course...</option>
                            @foreach ($courses as $course)
                                <option
                                    value="{{ $course->course_id }}" {{ ($paperSelection && $paperSelection->course_id == $course->course_id) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="batch_id" required
                                class="border border-yellow-400 w-full p-2.5 rounded outline-none focus:ring-2 focus:ring-purple-500 bg-white shadow-sm">
                            <option value="" disabled {{ !$paperSelection ? 'selected' : '' }}>Select Batch...</option>
                            @foreach ($batches as $batch)
                                <option
                                    value="{{ $batch->batch_id }}" {{ ($paperSelection && $paperSelection->batch_id == $batch->batch_id) ? 'selected' : '' }}>
                                    {{ $batch->name }}
                                </option>
                            @endforeach
                        </select>
